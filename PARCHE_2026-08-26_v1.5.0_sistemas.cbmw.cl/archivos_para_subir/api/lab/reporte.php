...
define('API_SECRET_TOKEN', 'LAYVEL_SECURE_TOKEN_2026_x89a');

// Validar Token de Seguridad API
$headers = function_exists('getallheaders') ? getallheaders() : [];
$receivedToken = $headers['X-API-Token'] ?? $headers['x-api-token'] ?? $_SERVER['HTTP_X_API_TOKEN'] ?? $_POST['api_token'] ?? '';

// Procesar Heartbeat enviado por el Agente
$input = file_get_contents('php://input');
$data = json_decode($input, true) ?: [];

if (!empty($data['api_token'])) {
    $receivedToken = $data['api_token'];
}

// Bloquear peticiones sin token válido
if ($receivedToken !== API_SECRET_TOKEN) {
    http_response_code(401);
    echo json_encode(['status' => 'error', 'message' => 'No autorizado: Token de API inválido o ausente']);
    exit;
}

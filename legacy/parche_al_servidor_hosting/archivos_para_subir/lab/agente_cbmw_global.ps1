...
function Desbloquear-Equipo {
    ...
    try {
        $hostsPath = "$env:SystemRoot\System32\drivers\etc\hosts"
        if (Test-Path $hostsPath) {
            $lines = Get-Content -Path $hostsPath -ErrorAction SilentlyContinue | Where-Object { $_ -notmatch "(roblox|minecraft)" }
            Set-Content -Path $hostsPath -Value $lines -Force -ErrorAction SilentlyContinue
            Log-Msg "   - Entradas de Roblox/Minecraft removidas de hosts." "Green"
        }
    } catch {}

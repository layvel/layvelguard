# ==============================================================================
# AGENTE GLOBAL DE AUDITORÍA, CONTROL Y LIMPIEZA CBMW (CON ANTI-DUPLICADOS)
...
function Obtener-Configuracion {
    ...
    # Configuracion fallback por defecto (Plataforma DIA + UMaximo)
    ...
    foreach ($s in $Shortcuts) {
        ...
        foreach ($desk in $desktops) {
            if (Test-Path $desk) {
                ...
                # COMPROBACIÓN ANTI-DUPLICADOS: Si ya existe el acceso directo, se omite
